...
impl OtpEngine {
    ...
    pub fn encrypt(&self, plaintext: &[u8]) -> Result<(u64, Zeroizing<Vec<u8>>), OtpError> {
        ...
        if key.len() < needed {
            ...
            // [SEC-FIX-D] エラーパスでも key を明示 drop してからconsume
            drop(key);
        ...
        // [SEC-FIX-D] 鍵を明示的に drop してゼロ化を強制する
        // Rust のドロップ順序はスコープ順のため、drop(key) を明示して
        // consume() 呼び出し前にゼロ化が完了していることを保証する
        drop(key);
        // コンパイラバリア: drop後のゼロ化がリオーダーされないよう保証
        core::sync::atomic::compiler_fence(core::sync::atomic::Ordering::SeqCst);
