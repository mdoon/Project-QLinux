...
impl KeyPoolManager {
    ...
    pub fn acquire(&self, needed: usize) -> Result<(u64, Zeroizing<Vec<u8>>), KeyPoolError> {
        ...
        if let Some(idx) = st.blocks.iter().position(
            ...
            // [SEC-FIX-A] ブロックをVecから完全に取り出し、
            // 不要バイトをゼロ化してから先頭neededバイトのみ返す
            // to_vec() は中間コピーを作りゼロ化されないため使用禁止
            let mut full_key = std::mem::replace(
                &mut st.blocks[idx].data,
                Zeroizing::new(Vec::new()),
            );
            // full_key[needed..] をゼロ化してから切り捨て
            // Zeroizing::new(full_key[..needed].to_vec()) は使わない:
            //   to_vec() が unzeroized な中間 Vec を Heap に作る
            for b in full_key[needed..].iter_mut() { *b = 0; }
            full_key.truncate(needed);
            return Ok((id, full_key));
        }
